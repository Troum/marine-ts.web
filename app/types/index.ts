...
/** Группа карточек внутри секции ЛНК (подсекция со своим заголовком). */
export interface LnkMarketingGroup {
  /** Заголовок подсекции. Если пусто — заголовок не отображается. */
  title: string
  /** Колонок в ряду на ≥lg для этой группы. Если не задано — используется columns секции. Диапазон: 1..6. */
  columns?: number
  cards: LnkMarketingCard[]
}

/** Секция ЛНК: заголовок, подсекции (группы карточек), число колонок по умолчанию. */
export interface LnkCardGridSection {
  title: string
  /** Колонок на широких экранах (1–6) по умолчанию (если у группы не задано своё значение). По умолчанию 3. */
  columns?: number
  /** Подсекции (группы) с карточками. Минимум одна (может быть с пустым title). */
  groups: LnkMarketingGroup[]
}

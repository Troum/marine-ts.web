...
const titleHint = ref('')
const translations = ref(mergeNewsTranslations())
...
onMounted(async () => {
  try {
    const item = await api.news.getById(id.value)
    titleHint.value = item.translations?.ru?.title ?? item.title
    translations.value = mergeNewsTranslations(item.translations)
  } catch {
    await navigateTo('/admin/seo/news')
  } finally {
    loading.value = false
  }
})
...
async function submit() {
  saving.value = true
  try {
    await api.news.update(id.value, { translations: translations.value })
    adminToast.success('SEO для новости сохранено')
    await navigateTo('/admin/seo/news')
  } catch {
  ...
  <div>
    <header class="sticky top-0 z-50 border-b border-mts-border bg-white">
      <div class="mx-auto flex h-16 max-w-[1600px] items-center px-6 lg:px-12">
        <NuxtLink to="/admin/seo/news" class="mr-4 text-mts-text-secondary hover:text-mts-accent">
          <ArrowLeft class="h-5 w-5" />
        </NuxtLink>
        <h1 class="font-display text-xl text-mts-text">SEO новости</h1>
      </div>
    </header>
    ...
    <main class="mx-auto max-w-[1600px] px-6 py-8 lg:px-12">
      ...
      <form v-else class="relative border border-mts-border bg-white p-8 shadow-tech" @submit.prevent="submit">
        ...
        <div class="mt-8 flex gap-4">
          ...
          <NuxtLink to="/admin/seo/news" class="btn-secondary">Отмена</NuxtLink>
        </div>
      </form>
    </main>
  </div>

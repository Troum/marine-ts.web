...
async function submit() {
  ...
  try {
    ...
    adminToast.success('Страница «Новости» сохранена')
  } catch {
  ...
  <div>
      ...
      <div class="max-w-[1600px] mx-auto px-6 lg:px-12">
        <div class="flex items-center justify-between h-16">
          <div class="flex items-center gap-4">
            ...
            <h1 class="font-display text-xl text-mts-text">Страница «Новости»</h1>
          </div>
          <div class="flex items-center gap-4">
            <NuxtLink to="/news" target="_blank" class="font-body text-sm text-mts-text-secondary hover:text-mts-accent transition-colors">Открыть на сайте ↗</NuxtLink>
    ...
    <main class="max-w-[1600px] mx-auto px-6 lg:px-12 py-8">
      ...
      <div v-else class="space-y-6">
        ...
        <!-- 2. Список (видимость и порядок относительно кастомных секций). -->
        <AdminCollapsibleSection
          ...
          hint="Сама лента новостей подгружается из админки «Новости». Здесь только управление видимостью и положением блока относительно кастомных секций."
        ...
        >
          <p class="font-body text-sm text-mts-text-secondary">
            Чтобы скрыть весь список новостей на публичной странице — снимите чекбокс «Показывать» в шапке этого блока.
          </p>
        </AdminCollapsibleSection>
        ...
        <section class="bg-white border border-mts-border shadow-tech relative p-6 space-y-4">
          <label class="flex cursor-pointer items-center gap-3 font-body text-sm text-mts-text">
            ...
            Показать форму заявки внизу страницы «Новости»
          </label>
          <label

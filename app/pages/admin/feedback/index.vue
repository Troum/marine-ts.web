...
async function fetchList() {
  pending.value = true
  try {
    items.value = await api.feedback.getManageAll({
  ...
  <div>
      ...
      <div class="mx-auto max-w-7xl px-6 lg:px-12">
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <h1 class="font-display text-xl text-mts-text">Обратная связь</h1>
          </div>
        </div>
      </div>
    </header>
    ...
    <main class="mx-auto max-w-7xl px-6 py-8 lg:px-12">
      <AdminListToolbar
        ...
        search-placeholder="Имя, email, текст…"
        :extra-filter="read"
        ...
        <table class="w-full min-w-[800px]">
          ...
          <tbody>
            <tr v-if="items.length === 0">
              <td colspan="6" class="p-8 text-center font-body text-mts-text-secondary">Сообщений пока нет</td>
            </tr>
            <tr
              v-for="m in items"
              :key="m.id"
              class="cursor-pointer border-b border-mts-border last:border-0 transition-colors hover:bg-mts-bg/60"
              @click="navigateTo(`/admin/feedback/${m.id}`)"
            >

...
import type { ContentPage, ContentPageContentableType } from '~/types'
...
const form = ref<Partial<ContentPage>>({
  slug: '',
  title: '',
  excerpt: '',
  body: '<p></p>',
...
onMounted(async () => {
  ...
  try {
    ...
    form.value = {
      ...
      body: normalizeBodyForEditor(item.body),
...
async function submit() {
  const bodyHtml = form.value.body ?? ''
  if (isRichTextEmpty(bodyHtml)) {
    await showAdminAlert({ message: 'Введите текст страницы', variant: 'error' })
    return
  }
  saving.value = true
  try {
    const base = {
      ...
      seoKeywords: form.value.seoKeywords?.trim() || null,
    }
    ...
    <main class="max-w-7xl mx-auto px-6 lg:px-12 py-8">
      ...
      >
        ...
        <div class="space-y-6">
          ...
          <div class="border-t border-mts-border pt-6">
            <p class="font-mono text-[10px] uppercase tracking-wide text-mts-text-secondary mb-4">SEO</p>
            <div class="space-y-4">
              <div>
                <label class="block font-mono text-[10px] uppercase tracking-wide text-mts-text-secondary mb-2">Meta title</label>
                <input
              ...
              <div>
                <label class="block font-mono text-[10px] uppercase tracking-wide text-mts-text-secondary mb-2">Meta description</label>
                <textarea
              ...
              <div>
                <label class="block font-mono text-[10px] uppercase tracking-wide text-mts-text-secondary mb-2">Keywords</label>
                <input
